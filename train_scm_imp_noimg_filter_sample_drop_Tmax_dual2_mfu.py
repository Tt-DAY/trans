...
def train(
    ...
    for epoch in range(start_epoch + 1, config.train.num_epochs + 1):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            target_route = sample_ordered_timesteps_from_ref(ref_route)
            ...
            if config.train.scm_loss:
                ...
                with torch.no_grad():
                    ...
                    for i in range(len(denoise_route)):
                        ti_target = target_route[i]
                        all_preds.append(x_t.detach())
                        # t = torch.tensor(ti.expand(x_t.shape[0]).view(-1, 1, 1, 1), device=x_t.device)
                        t_target = torch.tensor(ti_target, device=x_t.device).view(-1, 1, 1, 1)

                        all_t.append(t_target)
                        if config.train.scm_cfg_scale[0] > 1 and config.model.cfg_embed:
                            cfg_x_t = torch.cat([x_t, x_t], dim=0)
                            cfg_t = torch.cat([t_target, t_target], dim=0)
                            ...
                            pretrain_pred = pretrained_model(x_t / sigma_data, t_target.flatten(), **model_kwargs)
                        ...
                        if i == len(denoise_route) - 1:
                            ...
                            dt = target_route[i] - denoise_route[i+1]
                            x_denoise = torch.cos(dt) * x_t - torch.sin(dt) * dxt_dt
                            # noise = torch.randn_like(x_t)
                            noise = init_noise
                            dt2 = target_route[i+1] - denoise_route[i+1]
                            x_t = torch.cos(dt2) * x_denoise + torch.sin(dt2) * sigma_data * noise
            ...
            #     adv_loss = F.binary_cross_entropy_with_logits(pred_fake, torch.ones_like(pred_fake), reduction="none")
            ...
            # adv_loss = (adv_loss * black_mask.view(-1, 1, 1, 1)).mean()
            ...
            # with torch.no_grad():
            #     scm_cfg_scale = torch.tensor(
            #         np.random.choice(config.train.scm_cfg_scale, size=bs, replace=True), device=accelerator.device
            #     )
            #     data_info["cfg_scale"] = scm_cfg_scale
            ...
            # # cross entropy loss
            # if config.train.discriminator_loss == "cross entropy":
            #     loss_gen = F.binary_cross_entropy_with_logits(pred_fake, torch.zeros_like(pred_fake), redction="none")
            #     loss_real = F.binary_cross_entropy_with_logits(pred_true, torch.ones_like(pred_true), redction="none")
            #     loss_gen = (loss_gen * black_mask.view(-1, 1, 1, 1)).mean()
            #     loss_real = (loss_real * black_mask.view(-1, 1, 1, 1)).mean()
            #     loss_D = loss_gen + loss_real
            # # hinge loss
            # elif config.train.discriminator_loss == "hinge":
            #     loss_real = F.relu(1.0 - pred_true) * black_mask.view(-1, 1, 1, 1)
            #     loss_gen = F.relu(1.0 + pred_fake) * black_mask.view(-1, 1, 1, 1)
            #     loss_D = 0.5 * (loss_real.mean() + loss_gen.mean())
            # else:
            ...
            print(f"[{iter_time_end}] throughput per GPU (TFLOPS/s/GPU):{((1.024 + 0.56*(2*(valid_len+1) + 2*valid_len + 2*valid_len))/(iter_time_end - iter_time_start)):.2f},")
            if g_step % config.train.gradient_accumulation_steps == 0:
